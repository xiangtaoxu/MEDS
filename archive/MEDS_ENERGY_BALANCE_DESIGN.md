...
| **P2a** | **phase-change plateau** (inverter `_ON`, ice-aware `κ_sat(fliq)`/`C_eff(fliq)`) — self-contained, zero solver change, **unblocked** | 1,3,4,5 |
...
- **P2a — phase change (self-contained, unblocked).** Inverter plateau `_ON` + ice-aware `κ_sat(fliq)`/`C_eff(fliq)` — freeze/thaw automatic, **zero solver change** (a genuine in-module read-off change). Tests 1,3,4,5.
