...
and, on the **right** axis, (4) the **realized leaf litter** per month as **bars** — the leaf actually
shed. The realized litter is deliberately shown *separately* because it is **not** the shed tendency:

- a **bare** deciduous canopy has a high *winter shed tendency* but **zero** realized litter (nothing
  left to shed) — the litter bar peaks only in the autumn month when the canopy is actually dropping;
- a **full evergreen** canopy has a *zero shed tendency* but **non-zero** litter every month (baseline
  turnover of a full canopy);
- the **light-exchanger** has a modest shed tendency yet the **largest** litter flux of all four
  (~9 canopies/yr on the right-axis scale) — the canopy stays full while leaves turn over fast.

The driving environmental variable (air temperature / leaf ψ / radiation) is named in each panel title.
...
active shed, and how strategy 4's canopy (green) stays near 1 even as its litter bars swing with the
light — the signature of leaf exchange (high throughput, full canopy). Each panel's litter axis is
scaled independently (the litter flux spans ~400× from evergreen to leaf-exchanger).
