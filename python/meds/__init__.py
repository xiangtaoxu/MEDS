...
submodule:
    ...
    meds.pheno       — leaf-phenology signal kernel (two per-day flush/shed rate tendencies).
...
library lazily (meds.leaf / meds.pheno -> libmeds_plant_c, meds.demography -> libmeds_c) on first
import, so `import meds` works even without the shared libraries built.
"""
__version__ = "0.1.0"
__all__ = ["leaf", "pheno", "demography"]
