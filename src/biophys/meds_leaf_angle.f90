!==========================================================================================!
! meds_leaf_angle -- leaf-inclination distribution (LIDF) and the leaf-angle integrals of   !
! the SCOPE / 4SAIL radiative-transfer scheme (van der Tol et al. 2009; Verhoef 1984).       !
!                                                                                          !
! The LIDF is a two-parameter BETA distribution on the normalized inclination t = theta/(pi/2)!
! in [0,1] (Goel & Strebel 1984) -- a single generic family that subsumes the Campbell        !
! ellipsoidal and Verhoef archetypes (planophile / erectophile / plagiophile / extremophile / !
! uniform = Beta(1,1)). It is discretized onto the SCOPE 13-class inclination grid; class      !
! weights are differences of the regularized incomplete beta function (the Beta CDF).          !
!                                                                                          !
! Two quantities feed the two-stream optics (meds_canopy_optics):                             !
!   * bf     = <cos^2(theta_leaf)>  -- 2nd moment of leaf inclination; MU-INDEPENDENT, drives   !
!              the diffuse & beam back/forward scattering split. bf=1 horizontal, 0 vertical,   !
!              1/3 spherical.                                                                   !
!   * G(mu)  = <chi_s(theta_leaf, mu)> -- the exact Ross G-function (mean leaf-area projection   !
!              toward the sun) via the SCOPE `volscatt` projection; MU-DEPENDENT (per timestep).  !
!              The direct-beam extinction coefficient is k = G(mu)/mu.                            !
!==========================================================================================!
module meds_leaf_angle
