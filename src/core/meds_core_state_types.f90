...
module meds_core_state_types
   ...
   public :: GROWTH_AVG_UNSET, PSI_PREDAWN_UNSET, PSI_MAX_TODAY_RESET, PHENO_FLUSH_INIT, PHENO_SHED_INIT
   ...
   real(wp), parameter :: PSI_PREDAWN_UNSET = 1.0_wp
   ...
   real(wp), parameter :: PSI_MAX_TODAY_RESET = -1.0e30_wp
   ...
   type :: cohort_block
      ...
      !----- PLANT WATER STATUS driving the stomatal stress limb (issue #95). psi_leaf_predawn is    !
      ...
      real(wp),    allocatable :: psi_leaf_predawn(:)   !< [MPa] yesterday's daily-max psi_leaf (<= 0)
      real(wp),    allocatable :: psi_leaf_max_today(:) !< [MPa] running max over the current day
...
contains
   ...
   subroutine site_free(site)
      ...
      if (allocated(site%cohort%nplant)) deallocate(site%cohort%pft, site%cohort%nplant, site%cohort%dbh, &
         ...
         site%cohort%psi_leaf_predawn, site%cohort%psi_leaf_max_today,                            &
   ...
   subroutine cohort_alloc(cohort, cap, nwin)
      ...
      allocate(cohort%nplant(cap), cohort%dbh(cap), cohort%height(cap), cohort%basal_area(cap),            &
               ...
               cohort%psi_leaf_predawn(cap), cohort%psi_leaf_max_today(cap),                        &
      ...
      cohort%psi_leaf_predawn = PSI_PREDAWN_UNSET ; cohort%psi_leaf_max_today = PSI_MAX_TODAY_RESET
   ...
   subroutine cohort_ensure_capacity(cohort, need)
      ...
      tmp%psi_leaf_predawn(1:m)   = cohort%psi_leaf_predawn(1:m)
      tmp%psi_leaf_max_today(1:m) = cohort%psi_leaf_max_today(1:m)
   ...
   subroutine move_alloc_block(src, dst)
      ...
      call move_alloc(src%psi_leaf_predawn, dst%psi_leaf_predawn)
      call move_alloc(src%psi_leaf_max_today, dst%psi_leaf_max_today)
   ...
   subroutine cohort_reorder(cohort, perm, m)
      ...
      cohort%psi_leaf_predawn(1:m)   = cohort%psi_leaf_predawn(perm(1:m))
      cohort%psi_leaf_max_today(1:m) = cohort%psi_leaf_max_today(perm(1:m))
   ...
   subroutine copy_cohort_slot(cohort, dst, src)
      ...
      cohort%psi_leaf_predawn(dst)   = cohort%psi_leaf_predawn(src)
      cohort%psi_leaf_max_today(dst) = cohort%psi_leaf_max_today(src)
   ...
   subroutine init_cohort(cohort, m, pft, ipft, owner_patch, nplant, dbh)
      ...
      cohort%psi_leaf_predawn(m)   = PSI_PREDAWN_UNSET
      cohort%psi_leaf_max_today(m) = PSI_MAX_TODAY_RESET
