...
module meds_plant_vital_rates
   ...
   use meds_allometry, only : wood_to_dbh, dbh_to_agb, height_to_dbh
   implicit none
   private

   public :: carbon_growth_rate, camac_mortality, min_cohort_carbon, recruitment_contribution
...
contains
   ...
   elemental pure function carbon_growth_rate(wood_carbon, npp_wood, dbh, dt_yr,                &
   ...
   end function carbon_growth_rate
   ...
   !       establishment efficiency, over the min-size recruit carbon. Caller reduces these into   !
   !       the (PFT, patch) recruitment array and gates by include_pft.                            !
   elemental pure function recruitment_contribution(nplant, npp_repro, dt_yr,                   &
   ...
   end function recruitment_contribution
