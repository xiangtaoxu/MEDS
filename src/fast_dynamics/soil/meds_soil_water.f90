...
contains
   ...
   subroutine advance_soil_water_column(col, forcing, params, opts, dt, flux)
      ...
      !----- Ground evaporation (Philip alpha_soil + Swenson-Lawrence DSL), capped. Delegated to    !
      !      the public seam so the driver and this solve can never compute it differently. --------!
      e_soil = ground_evap_from_state(theta0(1), params, forcing, opts, dt)
   ...
   ! the IMEX-ARK fast integrator (docs/dev_plans/archive/MEDS_IMEX_ARK_DESIGN.md). Same flux-divergence + root    !
   ...
   !---------------------------------------------------------------------------------------!
   ! ground_evap_from_state -- the soil evaporation for a column at state^n, WITHOUT running the      !
   ! Richards solve.                                                                                  !
   !                                                                                          !
   ...
   ! (MEDS_PRODUCTION_INTEGRATOR_PLAN.md N2d/P2): the CAS needs the vapour source up front, but it     !
   ! does not need the Richards transport that used to be bundled with it.                             !
   !---------------------------------------------------------------------------------------!
   pure function ground_evap_from_state(theta1, params, forcing, opts, dt) result(e_soil)
      real(wp),               intent(in) :: theta1
      type(soil_params_t),    intent(in) :: params
      type(chydro_forcing_t), intent(in) :: forcing
      type(soil_opts_t),      intent(in) :: opts
      real(wp),               intent(in) :: dt
      real(wp) :: e_soil, psi1
      psi1 = soil_psi_from_theta(params%retention, theta1, params%theta_sat(1),                  &
                                 params%theta_res(1), curve_a(params, 1_ik), curve_n(params, 1_ik))
      e_soil = ground_evaporation(theta1, psi1, params, forcing, opts)
      e_soil = min(e_soil, max(0.0_wp, (theta1 - params%theta_res(1)) * params%dz(1)             &
                               * rho_h2o / dt))
   end function ground_evap_from_state
