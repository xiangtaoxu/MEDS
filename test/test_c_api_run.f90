...
!==========================================================================================!
! test_c_api_run -- COVERAGE FOR THE FULL-COUPLED-MODEL C-API SHIM (`meds.model`).           !
!                                                                                          !
! Two things here are load-bearing and neither is an assertion:                                !
!                                                                                          !
!   * `src/c_api/meds_c_api_run.f90` is COMPILED by a MANDATORY target, so a change to         !
...
program test_c_api_run
   ...
   use meds_c_api_run,     only : meds_run_open, meds_run_step, meds_run_is_done,                 &
   ...
   character(len=*), parameter :: CFG_LEGACY  = 'test_c_api_run_legacy_io.toml'
   ...
   !=== 2b. The [io] -> [state] RENAME keeps loading the old spelling (#173). ================!
   !        A user-visible config rename needs a deprecation path, and a deprecation path needs a
   !        test, or it rots into a block that silently reads nothing. This derives the legacy
   !        file FROM the shipped one, so it cannot drift: whatever [state] keys the shipped
   !        config carries are the ones checked under [io].
   block
      type(meds_config_t) :: cfg_new, cfg_old
      call load_meds_config(CFG_SPINUP, cfg_new)          ! shipped: [state]
      call write_legacy_io_config()                        ! same file with [state] -> [io]
      call load_meds_config(CFG_LEGACY, cfg_old)
      call check(trim(cfg_old%state_output_dir)    == trim(cfg_new%state_output_dir),             &
                 'legacy [io].output_dir loads to the same value as [state].output_dir')
      call check(trim(cfg_old%state_output_prefix) == trim(cfg_new%state_output_prefix),          &
                 'legacy [io].output_prefix still loads')
      call check(cfg_old%state_write_state .eqv. cfg_new%state_write_state,                       &
                 'legacy [io].write_state still loads')
      call check(cfg_old%state_interval_years_cfg == cfg_new%state_interval_years_cfg,            &
                 'legacy [io].state_interval_years maps to [state].interval_years')
      open(newunit=ulg, file=CFG_LEGACY, status='old', action='write') ; close(ulg, status='delete')
   end block
   ...
   print '(a)', 'test_c_api_run: ALL PASSED'
...
contains
   ...
   subroutine write_derived_config()
      ...
      write(uout,'(a)') '# GENERATED by test_c_api_run -- deleted when the test passes.'
      write(uout,'(a)') '[run]'
   ...
   !----- The shipped config with its [state] block renamed back to [io], key for key. Derived    !
   !      from the shipped file rather than written out by hand, so the legacy test cannot drift   !
   !      away from what the config actually contains (#173). ---------------------------------------!
   subroutine write_legacy_io_config()
      integer :: uin, uout, ios
      character(len=512) :: line
      open(newunit=uout, file=CFG_LEGACY, status='replace', action='write')
      write(uout,'(a)') '# GENERATED by test_c_api_run -- the shipped config in the DEPRECATED spelling.'
      open(newunit=uin, file=CFG_SPINUP, status='old', action='read')
      do
         read(uin,'(a)',iostat=ios) line
         if (ios /= 0) exit
         if (trim(adjustl(line)) == '[state]') then
            write(uout,'(a)') '[io]'
         else if (index(adjustl(line), 'interval_years') == 1) then
            write(uout,'(a)') 'state_'//trim(adjustl(line))
         else
            write(uout,'(a)') trim(line)
         end if
      end do
      close(uin) ; close(uout)
   end subroutine write_legacy_io_config
