...
program test_fusion_cohort
   ...
   use meds_diagnostics,     only : total_nplant, total_agb
   ...
   agb_tot = comm%coh%nplant(1)*comm%coh%agb(1) + comm%coh%nplant(2)*comm%coh%agb(2)
   ...
   call check_close(comm%coh%nplant(1)*comm%coh%agb(1), agb_tot, 1.0e-12_wp,                  &
                    'fused total AGB not conserved')
   call check_close(comm%coh%basal_area(1), pio4*comm%coh%dbh(1)**2, 1.0e-12_wp,              &
   ...
   do j = 1_ik, 200_ik
      ...
      call add_cohort(comm, cfg, 1_ik, 1_ik, 2.0e-4_wp, 2.0_wp + 0.55_wp*real(j-1_ik, wp))  ! 2..111 cm
   end do
   ...
   call check_close(total_agb(comm),    agb0, cfg%conservation_tol, 'fusion broke AGB conservation')
   ...
   call check_close(total_agb(comm),    agb0, cfg%conservation_tol, 'split broke AGB conservation')
