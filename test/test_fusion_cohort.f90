...
program test_fusion_cohort
   ...
   use meds_cohort_dynamics, only : fuse_2_cohorts, new_fuse_cohorts, split_cohorts, max_cohort_count
   ...
   ba_tot  = comm%coh%nplant(1)*comm%coh%basal_area(1) + comm%coh%nplant(2)*comm%coh%basal_area(2)
   dbh_avg = 0.5_wp*(comm%coh%dbh(1) + comm%coh%dbh(2))
   call fuse_2_cohorts(comm, 1_ik, 2_ik, cfg%conservation_tol)
   call check_close(comm%coh%nplant(1), 0.8_wp, 1.0e-12_wp, 'fused nplant must be summed')
   call check_close(comm%coh%nplant(1)*comm%coh%basal_area(1), ba_tot, 1.0e-12_wp,            &
                    'fused total basal area not conserved')
   call check_close(comm%coh%dbh(1), sqrt(comm%coh%basal_area(1)/pio4), 1.0e-12_wp,           &
                    'DBH must be re-derived from basal area')
   call check(abs(comm%coh%dbh(1) - dbh_avg) > 1.0e-6_wp, 'DBH must NOT be a plain average')

   !=== 2. new_fuse_cohorts reduces count to <= max_cohort, conserving N and BA. ===========!
   call init_bare_ground(comm, cfg, 1_ik, 298.15_wp, 295.15_wp)
   do j = 1_ik, 200_ik
   ...
   call check(max_cohort_count(comm) <= abs(cfg%max_cohort), 'fusion did not reach max_cohort')
   call check_close(total_nplant(comm),     n0,  1.0e-10_wp, 'fusion broke nplant conservation')
   call check_close(total_basal_area(comm), ba0, cfg%conservation_tol, 'fusion broke BA conservation')

   !=== 3. split_cohorts conserves N and BA and increases the count. ======================!
   call init_bare_ground(comm, cfg, 1_ik, 298.15_wp, 295.15_wp)
   call add_cohort(comm, cfg, 1_ik, 3_ik, 2.0_wp, 50.0_wp)   ! nplant*basal_area ~ 3927 > cap
   call finalize_init(comm)
   n0  = total_nplant(comm)
   ba0 = total_basal_area(comm)
   ...
   call check_close(total_basal_area(comm), ba0, cfg%conservation_tol, 'split broke BA conservation')
