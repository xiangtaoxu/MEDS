...
! {leaf balance, ground balance} -> canopy-air-space (CAS) three-twin update -> soil thermal      !
! column, driven by a diurnal cycle.                                                             !
!   1. CONSERVATION: every fast step closes the CAS energy / water / CO2 budgets AND the soil     !
!      thermal budget to machine precision (meds_budget_check n_fail == 0 over a full day).        !
!   2. PHYSICAL SANITY: CAS temperature tracks a diurnal cycle (warmer near solar noon); the       !
!      leaf warms under absorbed shortwave; CAS CO2 responds to the day/night biotic flux; and     !
!      the soil-surface diurnal temperature swing is DAMPED with depth (thermal inertia).          !
!==========================================================================================!
program test_column_dynamics
   ...
   use meds_column_dynamics,     only : column_config_t, column_forcing_t, column_fast_step
   implicit none

   integer(ik), parameter :: n = 1_ik, nsl = 10_ik, nstep = 96_ik    ! 96 x 900 s = 24 h
   real(wp),    parameter :: dt_fast = 900.0_wp, lat = 40.0_wp, t0 = 288.0_wp, tsoil0 = 288.0_wp
   ...
   type(budget_t)         :: be_e, be_w, be_c, be_s
   ...
   real(wp) :: ss_min, ss_max, sd_min, sd_max
   ...
   !----- Static column config: aero + veg-thermal defaults; build the soil column. -------!
   ...
   allocate(forc%abs_sw(n), forc%abs_lw(n), forc%gsw(n), forc%fs_open(n), forc%soil_water(nsl))
   forc%soil_water(1:nsl) = 0.30_wp

   !----- Seed the soil thermal column (internal energy + a valid initial diagnosed temp). -!
   do k = 1_ik, nsl
      bio%soil_e%soil_energy(k) = temp_to_uext(ccfg%soil_thermal%soil_dry_heat_capacity(k),     &
                                  forc%soil_water(k) * rho_h2o, tsoil0, 1.0_wp)
      bio%soil_e%soil_temp(k)   = tsoil0
   end do
   ...
   do istep = 1_ik, nstep
      ...
      forc%fs_open  = 1.0_wp
      forc%abs_sw_ground = 75.0_wp * cosz                        ! 15% of SW reaches the ground
      ...
      call column_fast_step(dt_fast, ccfg, aenv, ageom, n, lai, wai, height, crown, lwid, bdia, &
                            forc, bio, aero, be_e, be_w, be_c, be_s)
   ...
   !----- 1. Conservation: all four budgets closed every step. ----------------------------!
   ...
   call ck(be_s%n_fail == 0_ik, 'soil thermal budget closed every step', real(be_s%n_fail, wp))
   ...
   call ck(sd_min > 270.0_wp .and. sd_max < 330.0_wp, 'deep soil stays physical', sd_max)
