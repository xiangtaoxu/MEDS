...
> # ⚰️ RETIRED IN PART — 2026-09-13. **One of its "IMPLEMENTED" sections describes deleted code.**
>
> **Do not cite §3e, §5 or §14-P2.** This document records Zeng–Decker equilibrium correction, a
> lumped aquifer store, baseflow and a diagnosed water table as delivered in PR #23 — and they were
> all **deleted on 2026-07-30** (commit `55ffca0`, PR #88). The aquifer is now a **pure boundary
> condition**: a two-way head-driven flux against a saturated zone whose water table is *defined* as
> the column base, so the soil column is the unsaturated zone above it. The head-driven boundary
> supplies what Zeng–Decker reconstructed through the interior faces. `aquifer` is therefore the
> **wet-site** boundary condition — right for riparian and floodplain sites, wrong for upland.
>
> **Everything else shipped and is live** (PRs #22, #23, #61, #36): the implicit backward-Euler
> Thomas Richards solve with Celia modified-Picard linearization, upstream-weighted conductivity,
> adaptive step-doubling, conductivity-limited infiltration and ponding, dry-surface-layer soil
> evaporation, the ψ-limited root sink, and the van Genuchten / Campbell retention curves. The module
> is `meds_soil_water`, not the `meds_column_hydrology` named here.
>
> **Never built** (the document says so itself): the Neumann→Dirichlet ponded-surface switch,
> Green–Ampt infiltration, and the CTI saturated-fraction map.
>
> **Live description:** `docs/science/soil_biophysics.md`.
>
> Paths and line numbers below are the pre-2026-09 tree.
...
- **P2 — richer physics. IMPLEMENTED** (except the Dirichlet switch): Celia modified-Picard; **retention-integral Zeng–Decker equilibrium correction** (interior faces only, bottom-flux BC kept independent); **upstream-weighted interface K**; **adaptive step-doubling** (+ a fixed-count path); SIMTOP aquifer + water-table `z_wt` bottom BC (lumped store, baseflow, `z_wt` from specific yield); Dunne `f_sat` runoff. Validated: ZD holds a hydrostatic column at `q→0` while plain gravity drifts; Picard mass-closes + converges; adaptive matches a fixed-fine reference; aquifer store + water table respond and mass-close; Dunne runoff = `f_sat·precip`. **Still deferred:** the Neumann→Dirichlet ponded-surface top-BC switch (the interior solve + `θ`-clip handles surface saturation conservatively meanwhile).
...
- **MEDS internal:** `docs/dev_plans/archive/MEDS_HYDRAULICS_DESIGN.md` (§2/§5/§6.3/§11/§16 — the stateless + adaptive-step-doubling + boundary-only-conservation idiom this module reuses); `docs/dev_plans/archive/radiative_transfer_design.md` (the `meds_biophysics` seam/DAG precedent, incl. `derive_rad_optics` filling `rad_pft_optics_t`); `CLAUDE.md` (state/process wall, no-hard-coded-parameters, naming convention, issue #7 nvfortran trap).
