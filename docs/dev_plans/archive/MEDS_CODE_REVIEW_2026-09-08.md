...
> # 🗃️ ARCHIVED — 2026-09-13. Executed; its remainder was re-homed.
>
> **Items 1A, 1B #1–#3, 2 and 4–6 shipped in PRs #119, #120, #122, #123 and #124** (2026-09-08/09).
> **Items 1B #4–#10 became the slow-loop conservation ledger**, PRs #132–#137. The header's claim
> that items 4–10 are "NOT covered" is stale — items 4–6 are executed in this same file's own logs.
>
> **What was left over was folded into `MEDS_CODE_STRUCTURE_DESIGN.md`** §15.6 and §15.7, which is
> the live document: per-layer face budgets, the `column_cohort_t` removal, and consolidating the
> per-test `check` routines. See also `docs/ROADMAP.md` §10.
>
> Headline number: a 50-year spin-up's whole-column energy imbalance went from +6.75 MJ m⁻² to
> 3.6×10⁻¹¹ W m⁻².
>
