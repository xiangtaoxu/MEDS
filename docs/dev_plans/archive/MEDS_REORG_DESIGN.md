...
> # 🗃️ ARCHIVED — 2026-09-13. **Its status header is wrong — read this first.**
>
> **The header below says "DESIGN-ONLY … not implemented, no branch". That is false.** P0–P4 were
> implemented and merged as **PR #43 on 2026-07-16**, the same day the document was written. Anyone
> reading the header alone gets the history backwards.
>
> **What shipped and survives:** the carbon→structure composite in the shared allometry, the
> `update_overtopping_lai` competition sweep, the empirical vital-rate laws moved out to the Python
> example, and the Python companion library (now a single `libmeds.so`, PR #138).
>
> **What was superseded:** its module placement and library boundaries, twice — first by
> `archive/MEDS_CORE_MODULE_REORG_DESIGN.md` (PR #44) and then by
> `MEDS_CODE_STRUCTURE_DESIGN.md` (PRs #125–#127), which is the live structure document.
>
