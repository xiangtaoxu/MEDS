...
> # 🗃️ ARCHIVED — 2026-09-13. Implemented, then replaced, then deleted.
>
> **This was not design-only.** Phases P3a–d shipped in **PR #37** (2026-07-09) as a Picard
> iteration over the leaf ↔ canopy-air ↔ ground ↔ soil surface. It was superseded on 2026-07-11
> by the **Newton arrowhead solve** in the ESDIRK2 stage (`MEDS_IMEX_ARK_DESIGN.md`), removed with
> the operator-split integrator in **PR #88**, and its five `[fast].picard_*` config keys deleted in
> **PR #105**. P3e and P3f were never built.
>
> The coupled surface fixed point that this document argues for **does exist** in MEDS today — it is
> just solved by a 2×2 Newton iteration inside the implicit stage rather than by outer Picard
> relaxation.
>
> **Live description:** `docs/science/column_biophysics.md`. Module:
> `src/fast_dynamics/numerics/meds_fast_be_stage.f90`.
>
...
fluxes as its source — seams #8/#9 of `docs/dev_plans/archive/MEDS_ENERGY_BALANCE_DESIGN.md` §9).
...
coupled solve with **"only the driver stops lagging"** (`docs/dev_plans/archive/MEDS_ENERGY_BALANCE_DESIGN.md` §6.3).
