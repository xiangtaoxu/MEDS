...
> # 🗃️ ARCHIVED — 2026-09-13. Part I's layout is superseded; Part II shipped.
>
> **Part I** — the flattening of `src/plant/` into one ecophysiology library behind a
> `meds_plant_interface` façade, with allometry as its own `src/allometry/` library — shipped on
> 2026-07-04 and was then **replaced wholesale** by `MEDS_CODE_STRUCTURE_DESIGN.md` (PRs #125/#126,
> 2026-09-09). The plant kernels now split by **timescale** into `src/fast_dynamics/plant/` and
> `src/slow_dynamics/plant/`, the façade was deleted, and allometry lives in `src/functions/`.
>
> **Part II** — non-leaf maintenance respiration — shipped in **PR #13** (2026-07-04) and is live.
> `growth_respiration` was later relocated into the daily allocator (PR #52), where it is charged on
> *realized* growth.
>
> **Its §11–§12 respiration equations are no longer the only record:** they are now in
> `docs/science/plant_respiration.md`, written specifically so this document could be archived. The
> §14 `[respiration]` option block and its 15→25 °C converter were never built and are moot — the
> shipped parameters are already 25 °C-referenced per-PFT traits.
>
> **Still deferred** (see `docs/ROADMAP.md` §7): thermal acclimation, storage respiration, layered
> root respiration.
>
