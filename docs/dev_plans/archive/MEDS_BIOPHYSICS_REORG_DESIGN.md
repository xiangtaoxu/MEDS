...
> # 🗃️ ARCHIVED — 2026-09-13. Shipped, then superseded by the tree-wide reorganization.
>
> **Shipped in PR #57** (2026-07-21): the fast-loop kernels regrouped by surface subsystem, with a
> shared optics library and a library interface façade.
>
> **Every module it created has since moved or been renamed** by
> `MEDS_CODE_STRUCTURE_DESIGN.md` (PRs #125–#127): the façade was deleted, `meds_biophysics_types`
> dissolved into `meds_canopy_types` / `meds_soil_types` / `meds_plant_types`, the snow kernels
> merged into `meds_ground_biophysics`, and `meds_optics_lib` moved to `src/functions/`.
>
> **The grouping principle it established — one module per thermal or chemical store — is live** and
> is restated in `src/README.md`.
>
