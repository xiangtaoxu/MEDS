...
>
> **Shipped 2026-07-13** (commits `01347bb`, `518f08d`, merged directly with no PR number). The
> FAST (sub-daily) netCDF tier, `fast_interval_steps`, the per-day file chunking and the
> `[fast].fast_probe` debug switch are all live.
>
> **Its deferred P2** — per-cohort FAST variables — was delivered differently, by the v0.1
> diagnostic layer (`MEDS_IO_V01_PLAN.md` decision D1), which emits them as fast-window means
> through the general registry.
>
> **One thing it planned was not done and is still open:** unifying the FAST tier's bespoke staging
> path (`fast_sample_t`, `extract_fast_scalar`, `output_integrate_fast`) into the general
> `extract_variable` switchboard. Tracked in `docs/ROADMAP.md` §6.
>
> **Live description:** `docs/science/diagnostics.md`.
>
> Its line-number citations into the output modules are the pre-2026-09 tree.
