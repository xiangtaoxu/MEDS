# MEDS source-tree reorganization — migration log

> # 🗃️ ARCHIVED — 2026-09-13. The record of how the 2026-09 reorganization ran.
>
> **Split out of `MEDS_CODE_STRUCTURE_DESIGN.md`**, which keeps the decisions, the placement rules
> and the open items. This file holds the parts that are history: why the old tree read awkwardly
> (§2), the lever that dissolved the constraint (§3), the target tree as specified before it was
> built (§4), the deployment-cost analysis (§7.3–§7.5), the migration order (§8), the relation to
> other plans (§9), the review-remainder folding (§10.1, §10.3–§10.6), and the four sections
> recording what implementing each step changed about the plan (§11–§14).
>
> **Section numbers are the originals**, so the ~40 source comments that cite this plan by section
> number still resolve. They are not contiguous in this file.
>
> **The tree it specifies in §4 is not the tree that exists.** Nine modules were renamed and seven
> added after §4 was written; `src/README.md` is the live description. Every path and line number
> below is the pre-2026-09 tree except where a §11–§14 section says otherwise.
>
> Shipped as PRs #125 (steps 1–6), #126 (steps 9–10), #127 (steps 0 and 7) and #138 (step 8).

**Verification, as recorded when the migration closed.** Every implemented step was verified on
**both** back ends (ifx 38/38 and nvfortran 38/38 multicore) and **byte-identical** in all 75 netCDF
outputs of a 3-year, 4-thread reference run — including the two module *splits* (steps 2 and 4),
which the plan had expected to be only data-identical at round-off. §13 records a verification gap
found afterwards that affects how §8's acceptance criterion should be read.

**Provenance.** Written 2026-09-08, revised after review PRs #119–#124, with the naming pass on
2026-09-09 settling the folder names with the author: `fast_dynamics/` and `slow_dynamics/`,
`demography/` reinstated for the operator half of what had briefly been `core`, `main/`, and the
vital-rate laws moving to demography. §10 folds the still-open items of
`MEDS_CODE_REVIEW_2026-09-08.md` into the migration order so the two plans read as one sequence.
...
- `archive/MEDS_DRIVER_REORG_DESIGN.md` — step 7 extends it.
- `MEDS_CAPI_REORG_DESIGN.md` — §7.6 extends it. **This one is deliberately untracked**: it is a
  local working document, kept on disk and excluded in `.gitignore`, so the reference resolves only
  in the author's checkout. Its one load-bearing decision — a single `libmeds.so` rather than one
  shared library per domain — is decision **#1** of this plan and shipped in PR #138; its Part B
  nine-file C-API split was considered and rejected in §14.2. Nothing in a clone needs it.
