...
> # 🗃️ ARCHIVED — 2026-09-13. **Its "design-only" header is stale; this shipped the same day.**
>
> **Merged in PR #49 on 2026-07-18**, the date on the header below. `src/functions/meds_hydr_lib.f90`
> *is* this extraction: the pressure-volume curves, the xylem vulnerability curve and its Kirchhoff
> potential, and the fixed-grid lookup table all moved down into the shared functions layer, leaving
> the matrix-exponential solver in the plant module. The soil retention curves were later added
> beside them, which is the point the design makes — they are the same kind of object.
>
> **The module is `meds_hydr_lib`, not the `meds_hydro_curve` this document names.**
>
> Per-PFT hydraulic traits (§8) were not done: the parameters are still PFT-uniform
> (`docs/ROADMAP.md` §7).
>
