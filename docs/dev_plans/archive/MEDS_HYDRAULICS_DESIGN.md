...
>
> **P0–P3 shipped in PR #9** (2026-07-04) and the refactor in **PR #49** (2026-07-18), which moved
> the constitutive curves into the shared `meds_hydr_lib` and added the multi-layer root boundary.
> The C-API and Python path followed. The types live in `meds_plant_types`, not the `meds_hydro_types`
> named here.
>
> **Its §14-P5 leaf ↔ hydraulics Picard coupling was solved differently:** by the ESDIRK2 stage plus
> the transpiration corrector of PR #91, which removed a pure flux inconsistency — exactly
> `dt·(transp_pp − transp_bw)` — and improved `psi_leaf` convergence 314×.
>
> **Never built:** `stem_psi_at_height` (§6.4). Still deferred: per-layer root nodes and hydraulic
> redistribution (`docs/ROADMAP.md` §7).
>
> **Live description:** `docs/science/plant_hydraulics.md`.
>
