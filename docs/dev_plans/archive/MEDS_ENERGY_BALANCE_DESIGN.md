...
> # 🗃️ ARCHIVED — 2026-09-13. Complete; its P3 was solved by a different method.
>
> **P0/P1 shipped in PR #24 and P2a (the freeze/thaw plateau) in PR #25** (2026-07-05/06), with the
> TOML surface in PR #61. The governing decision — every store carrying **internal energy or
> enthalpy rather than temperature**, so phase change is a read-off of an invertible map instead of
> a special case — is the backbone of the fast loop and is unchanged.
>
> **Its §6.3 / §14-P3 coupled surface fixed point exists, but not as designed.** This document plans
> an outer Picard iteration; MEDS solves the leaf ↔ canopy-air coupling with a **2×2 Newton
> arrowhead** inside the ESDIRK2 implicit stage.
>
> **Two renames:** `uext_to_temp` is `internal_energy_to_temp`, and the thermodynamics module is
> `meds_therm_lib`. The `[energy].phase_change` flag it introduced was deleted in PR #143 —
> ice-aware soil properties are unconditional.
>
> **Live description:** `docs/science/column_biophysics.md` with its per-store pages
> (`canopy_air_space_biophysics.md`, `soil_biophysics.md`, `vegetation_energy_dynamics.md`,
> `snow_biophysics.md`). Still deferred: the explicit skin layer and the multi-layer canopy air space
> (`docs/ROADMAP.md` §5, §3).
>
