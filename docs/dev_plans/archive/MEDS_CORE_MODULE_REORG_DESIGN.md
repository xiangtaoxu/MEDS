...
> # 🗃️ ARCHIVED — 2026-09-13. **Its status header is wrong, and its central rename was reversed.**
>
> **The header below says "design-only". That is false.** Phases A–E merged as **PRs #44 and #45 on
> 2026-07-16/17**.
>
> **Its defining decision was undone.** This plan renamed the demography library and its modules to
> `core`. On 2026-09-09 (PR #126) every one of those names was renamed **back** to `demography`:
> once the state half became its own `state/site` layer, `core` stopped carrying any information.
> Nothing in the tree is called `core` today, so every `meds_core_*` name below — and there are many
> — names a module that does not exist.
>
> **What survives:** the applier/state split itself (`update_cohort_states`, `update_patch_states`,
> the `cohort_deriv_block` tendency bundle, `init_cohort`) and the `meds_io_prep` / `meds_io_stream`
> target split, all live.
>
> **Live structure document:** `MEDS_CODE_STRUCTURE_DESIGN.md`.
