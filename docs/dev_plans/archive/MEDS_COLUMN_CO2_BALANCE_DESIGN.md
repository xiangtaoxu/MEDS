...
> # 🗃️ ARCHIVED — 2026-09-13. The twin shipped; the document's placement argument was overturned.
>
> **The prognostic canopy-air CO₂ twin shipped in PR #26** (2026-07-06), with the DAMM option in
> PR #27. It is live — but under none of the names this document uses: the kernels are
> `cas_column_time_deriv` / `cas_column_step_implicit` in
> `src/fast_dynamics/canopy/meds_cas_biophysics.f90`, sharing one box with the enthalpy and humidity
> twins.
>
> **Its §2 placement decision was reversed.** This document argues that one `biogeochemistry`
> library should own both the fast and slow halves of the carbon cycle. The tree is timescale-first
> now: the fast CO₂ exchange is a diffusion/venting process and lives in `fast_dynamics/canopy/`,
> while the slow CENTURY matrix lives in `slow_dynamics/soil/`. **Heterotrophic respiration is the
> single documented fast↔slow seam**, and co-locating it with the daily step is what closes
> `rh_seam_gap` to machine precision.
>
> **Two kernels here are unreachable:** `heterotrophic_respiration_flux` (Q10) and
> `heterotrophic_respiration_damm` both exist and are tested but have no config key and no caller —
> the matrix form is the single Rh authority. Tracked in `docs/ROADMAP.md` §3.
>
> **Live description:** `docs/science/canopy_air_space_biophysics.md` and `soil_carbon.md`. Its
> §3.5 multi-layer canopy air space is still a candidate (`docs/ROADMAP.md` §3).
>
