...
> # 🗃️ ARCHIVED — 2026-09-13. Complete; kept because 50+ source comments cite its phase numbering.
>
> **Its header says "not yet PR'd"; that is stale.** P0–P4 merged in **PR #67** (2026-07-24) and
> P5/P6 robustness, including the uninitialised read that hung the 30-year run, in **PR #68**
> (2026-07-27). The document's entire planned scope shipped.
>
> **Three names changed on the way in.** `INTEG_RK4` shipped as `INTEG_RK45`. The stiff-rescue
> target was the operator-split scheme and is now **ARK**, because split was retired in PR #88.
> `rk45_refresh_aero` was never built under that name — the per-stage Monin-Obukhov refresh in
> PR #90 supersedes it.
>
> **What this document is still the record of:** internal plant water carried as **mass** rather
> than potential, the mass-conserving demographic seam, and the phase numbering (P0…P6) that source
> comments in `meds_fast_rk45.f90`, `meds_fast_frozen.f90`, `meds_fast_types.f90` and the column
> tests refer to by number.
>
> **Live description:** `docs/science/numerical_scheme.md` §3 and `plant_hydraulics.md`.
>
