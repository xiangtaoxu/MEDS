...
> # 🗃️ ARCHIVED — 2026-09-13. Phase A shipped and the opt-in flag is gone.
>
> **Phase A merged in PR #49** (2026-07-18). The `[hydraulics].multilayer_roots` flag was **deleted**
> on 2026-07-30, making per-layer soil potentials and conductivity-weighted rhizosphere conductances
> the only path — so the "bit-identical when off" property this document designs for no longer
> applies, and the single root-fraction-weighted boundary is gone.
>
> **Hydraulic redistribution is deliberately not enabled:** per-layer root efflux is floored at zero
> in both the plant solver and the soil sink, so uptake is non-negative and conserved by
> construction.
>
> **Still open:** Phase B (per-layer root nodes in the hydraulic network) and hydraulic
