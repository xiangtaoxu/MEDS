...
> # ⚰️ RETIRED — 2026-09-13. Its core algorithm no longer exists. Do not read §4 as current.
>
> **This document designs a directional tri-state phenology** (`PHEN_ON` / `PHEN_OFF` /
> `PHEN_DORMANT` driving a `target_frac`). That design shipped in **PR #10** (2026-07-04), was wired
> into the run loop in **PR #39**, and was then **replaced** on 2026-07-19 by the rate-based kernel
> in `MEDS_PHENOLOGY_RATE_REFACTOR_DESIGN.md` (PR #51), which emits **two relative rate tendencies**
> and never touches leaf mass. None of §4's engine equations describe the model any more.
>
> **What survives:** the cue taxonomy, the derived-type names (`pheno_env_t` / `state_t` /
> `params_t` / `out_t`), the test file and the C-API. Its §10 verification recipe cites the
> `-DMEDS_ENABLE_IO=OFF` build, which was removed.
>
> **Read instead:** `docs/science/plant_phenology.md`, and
> `MEDS_PHENOLOGY_RATE_REFACTOR_DESIGN.md` for the live plan (its P3–P5 are still open — see
> `docs/ROADMAP.md` §2).
...
Template/style: `docs/dev_plans/archive/MEDS_HYDRAULICS_DESIGN.md`. Kinds `wp/ik`, `_wp` literals, `implicit none`,
...
| `src/plant/phenology/README.md` + `docs/dev_plans/archive/MEDS_PHENOLOGY_DESIGN.md` | seam doc + this design | hydraulics README + design |
...
- Template: `docs/dev_plans/archive/MEDS_HYDRAULICS_DESIGN.md`.
```
