...
> # 🗃️ ARCHIVED — 2026-09-13. Complete.
>
> **Shipped in PR #5** (2026-07-03). Its line 14 — "the code does not exist yet" — and its
> `biophys:` title prefix are both stale: the directory was renamed `biophysics` within days and the
> modules now live in `src/fast_dynamics/canopy/`.
>
> **The six modules it planned collapsed into two:** the pure optical-property kernels (leaf-angle
> distribution, the Ross G function, the single-scatter pair) are in the shared
> `src/functions/meds_optics_lib.f90`, and the assembly, the unified multi-band adding solver and the
> public seam are together in `meds_canopy_radiation`.
>
> **Its deferred list has partly closed:** the wood-area allometry is now ED2's real `b1WAI`/`b2WAI`
> rather than a fraction of leaf area, and ground optics including the snow-albedo ramp are wired.
> Finite crowns, BRDF and multiple-scattering variants remain declared non-goals.
>
> **The ED2 two-stream bugs found during the port are issue #6.**
>
> **Live description:** `docs/science/canopy_radiation_transfer.md`.
>
