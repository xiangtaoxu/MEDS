...
> # 🗃️ ARCHIVED — 2026-09-13. Complete; kept as the record of how the schemes were reconciled.
>
> **Phases 0–3 shipped in PR #88** (2026-07-31): the `multilayer_roots` flag deleted and the
> per-layer path made unconditional, the aquifer rebuilt as a head-driven boundary condition, and
> Dunne runoff, the diagnosed water table and the Zeng–Decker correction all deleted. **Phase 4**
> (prognostic wood on ARK and RK45) also landed in PR #88, and **Phase 5** was realised differently
> — by the exact-exponential tissue store in `MEDS_VEG_ENERGY_INTEGRATION_PLAN.md`, not as planned
> here. §7 of this document still says Phases 4 and 5 are not implemented; that is stale.
>
> **One residue outlives the plan:** the `--parity` preset in `scripts/numerics_sweep.py` pins two
> config keys that no longer exist, so it cannot run. Tracked in `docs/ROADMAP.md` §12.
>
> **Live description:** `docs/science/numerical_scheme.md` §4.
>
