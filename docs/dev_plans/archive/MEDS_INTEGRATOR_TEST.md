...
> # ⚰️ RETIRED — 2026-09-13. Do not use this document to justify a decision.
>
> **Its 24-run comparison matrix is void.** Every run was scored against the operator-**split**
> integrator, which was retired on 2026-07-31 precisely because it converged to a different,
> never-attributed limit (~0.45 K in canopy-air temperature). "Agrees with split" measures a
> distance from an unknown, not an error.
>
> **Independently, every measurement sits inside an oscillation.** The matrix was taken at
> `dt_fast` 600–1800 s, inside the frozen-conductance regime that drove a sustained period-2
> canopy-air oscillation (~8 K peak-to-peak at 900 s) which no conservation ledger detects. The
> accuracy numbers are phase-samples, not convergence measurements. The cause was found and fixed
> in PR #90 by refreshing the canopy-air-to-atmosphere conductance every integrator stage.
>
> **Its headline recommendation — 900 s, ARK adaptive — happens to match today's default, and that
> is a coincidence.** Today's 900 s is an *accuracy* choice arrived at after the oscillation was
> removed; this document's 900 s was a *stability* compromise inside it. The "SPLIT is most accurate
> on energy and water" finding, the latent- and sensible-heat numbers, and the cost ratios are all
> void.
>
> **What survives:** the FAST output tier is a real diagnostic axis and is still used (§9).
>
> **Read instead:** `docs/science/numerical_scheme.md` §5, and
> `MEDS_PRODUCTION_INTEGRATOR_PLAN.md` §1h–§1i for the measurements that replaced these.
...
(`docs/dev_plans/archive/MEDS_FAST_OUTPUT_DESIGN.md`), so the 1-hour comparison axis this evaluation needs already exists
