...
> # 🗃️ ARCHIVED — 2026-09-13. **Its "design-only" header is stale; both parts shipped.**
>
> **Part I merged as PR #63 and Part II as PR #64**, both on 2026-07-22.
>
> **Part I is where the run-model decision was taken**, and that decision still governs MEDS: the
> fast biophysics loop is **always on**, leaf phenology is **unconditional**, and a slow-only
> empirical-demography run is the Python C-API path rather than a Fortran branch. The `phenology_on`
> flag was retired here.
>
> **Part II wired the CENTURY soil carbon into the slow loop**: the thin `meds_slow_dynamics`
> coordinator sequencing vegetation dynamics and biogeochemistry as peer domains, the per-patch
> state, the `[soil_carbon]` TOML block, netCDF restart, and the litter → daily step → fast-Rh seam
> whose gap closes to machine precision.
>
> **Caution:** its many `src/driver/…#L` links are dead — that directory no longer exists, and the
> line anchors are the pre-2026-09 tree. Eighteen source comments cite this document by name.
>
> **Live description:** `docs/science/soil_carbon.md` and `plant_phenology.md`.
