...
> # 🗃️ ARCHIVED — 2026-09-13. Complete; kept as the design record of the production integrator.
>
> **Shipped 2026-07-11** (commit `e77f7f0`, merged directly with no PR number), made the default by
> **PR #88** (2026-07-31) when the operator-split scheme was retired. Phases P0–P5 all landed.
>
> **Two things here differ from what shipped.** The tableau is **ARS(2,2,2)**, not the
> ARK2(1)3L[2]SA this plan specifies. And "IMEX-ARK" is a misnomer that stuck: the biotic CO₂
> source is folded implicit, so the explicit part is empty and the scheme is a 2-solve **ESDIRK2**
> (γ = 1 − 1/√2). The config string is still `"ark"`. **P6** (folding the soil-water top node into
> the tableau, and ARK3(2)4L[2]SA) was never built and is explicitly rejected in
> `MEDS_PRODUCTION_INTEGRATOR_PLAN.md` §8.
>
> **Live description:** `docs/science/numerical_scheme.md` §3. Open numerics work:
> `docs/ROADMAP.md` §4. Module: `src/fast_dynamics/numerics/meds_fast_ark.f90`.
>
