...
> # 🗃️ ARCHIVED — 2026-09-13. Superseded by the exact-exponential tissue store.
>
> **P0–P3 shipped in PR #41** (2026-07-13): a separate prognostic wood temperature, separate
> longwave emission, and the L-stable fix for the young-wood blow-up.
>
> **P4 was abandoned.** The bordered-arrowhead prognostic-leaf solve was never built. It was
> superseded by `MEDS_VEG_ENERGY_INTEGRATION_PLAN.md` (2026-07-31), which found the tissue ODE is
> **linear** under the frozen coefficients and therefore has an exact exponential solution with two
> weights — an endpoint weight for the committed state and a step-average weight for every reported
> flux. Pairing them is what makes the balance close identically. The `leaf_energy_model` /
> `wood_energy_model` selectors this document introduced were deleted: "diagnostic" is the
> zero-heat-capacity limit of the one formula, not a separate mode.
>
> **One of its measurements was later corrected:** the wood time constant is
> $`\tau_{\mathrm{wood}} \approx 10^4 r`$ seconds in stem radius, so wood is **stiff** for most
> cohorts — this document's "non-stiff" reading is wrong.
>
> **Live description:** `docs/science/vegetation_energy_dynamics.md`.
>
