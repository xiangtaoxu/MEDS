...
>
> **The header below says the plan was "not executed". That is false.** Phases 1–5 merged as
> **PR #62 on 2026-07-21**: the fast-loop modules were renamed and split into per-scheme back ends
> (`meds_fast_dynamics`, `meds_fast_types`, `meds_fast_time_derivs`, `meds_fast_ark`,
> `meds_fast_rk4_oracle`) and the pre-pass was de-duplicated. Its §8 slow-loop split shipped in
> PR #64.
>
> **The layout it produced was then re-laid-out** by `MEDS_CODE_STRUCTURE_DESIGN.md` (PR #125):
> `src/driver/` no longer exists, and those modules now live in `src/fast_dynamics/{numerics,driver}/`.
>
> **One decision was never taken:** §7 proposes retiring the IMEX-Euler tier, which still exists in
> `meds_fast_rk4_oracle.f90`. Tracked in `docs/ROADMAP.md` §12.
>
