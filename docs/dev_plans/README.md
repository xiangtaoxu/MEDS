...
| [`MEDS_FORCING_DESIGN.md`](MEDS_FORCING_DESIGN.md) | Part II (§11–§19, revised 2026-09-26): the ERA5-Land archive (download tools ✅ F1; global per-variable monthly `ED_ERA5land_` archive F2–F3; the reader upgrade F4: `met_source`, `data_path`, site/box monthly reads, internal conversion of dewpoint and wind components; later products). Also open: transient CO₂ (#184), retiring the `apply_met_to_ctx` shim, the diagnostic forcing echo. Reference for the `legacy_file` format (§7.1) and the de-accumulation recipe (§7.3). The multi-polygon runtime moved to `MEDS_POLYGON_RUNTIME_PLAN.md`. |
...
| [`MEDS_POLYGON_RUNTIME_PLAN.md`](MEDS_POLYGON_RUNTIME_PLAN.md) | Design only (2026-09-26). Regional runs as an OpenMP loop over independent polygons, without MPI: month-synchronous compute/IO split, shared forcing month buffer, polygon-dimension output, ragged restart, batching and job arrays for large domains. Phases R0–R6; the non-MPI part of ROADMAP #183. Needs the forcing reader upgrade (`MEDS_FORCING_DESIGN.md` F4, §15). |
