...
- **MEDS internal:** `docs/dev_plans/MEDS_HYDRAULICS_DESIGN.md` (§2/§5/§6.3/§11/§16 — the stateless + adaptive-step-doubling + boundary-only-conservation idiom this module reuses); `docs/dev_plans/radiative_transfer_design.md` (the `meds_biophysics` seam/DAG precedent, incl. `derive_rad_optics` filling `rad_pft_optics_t`); `CLAUDE.md` (state/process wall, no-hard-coded-parameters, naming convention, issue #7 nvfortran trap).
