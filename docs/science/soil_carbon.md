...
$`u = B\,I`$ is the litter input (§5); $K$ is the diagonal of baseline decay rates, carried in
$`[\mathrm{yr^{-1}}]`$ by the config and divided by `yr_day` at assembly so the kernels run in
$`[\mathrm{day^{-1}}]`$; $\xi$ is the diagonal per-pool environmental scalar (§4); $A$ the
dimensionless donor/transfer matrix (§3). The input is **litter carbon, never GPP** — autotrophic
respiration was removed upstream in the plant carbon budget.
...
into the type but inert (§12); `pack_pool_vector` / `unpack_pool_vector` marshal the named fields to
...
**Default scheme, three active chemistries.** Metabolic litter respires 100% and transfers nothing;
structural sends its complement to slow ($`a_{6,3}=1-e_{r,3}`$, $`a_{6,4}=1-e_{r,4}`$); slow is the sole
SOM outlet and respires 100%. The structural respired fraction is lignin-weighted,
...
**Scheme 5, five active pools** (Bolker/Pacala/Parton CENTURY). Fast litter → microbial; structural
non-lignin → microbial, lignin → slow; microbial ⇄ slow and slow ⇄ passive back-transfers,
**texture-controlled**: sand sets the microbial respired fraction
$`e_{r,\mathrm{micr}}=\texttt{er\_micr\_int}+\texttt{er\_micr\_slp}\cdot x_{\mathrm{sand}}`$, clay the
fractions routed to passive, $`\min(1-e_r,\ \texttt{int}+\texttt{slp}\cdot x_{\mathrm{clay}})`$. Those
back-transfers sit above the diagonal, so scheme 5's active block is genuinely non-triangular (§9).
...
\exp[(m_{\mathrm{opt}}-m)\,b_{\mathrm{wet}}] & m > m_{\mathrm{opt}}\end{cases} \qquad(3)
```
...
1 & \text{otherwise}\end{cases} \qquad(4)
```
...
formula. The tests assert it: the correct form's total loss equals $`R_h`$ of eq (2); the other does not.

## 5. Litter input

Dead plant carbon becomes litter in one place — `necromass_to_litter` (`meds_litter_partition`), whose
arguments are plain carbon amounts and plain PFT traits, which is what lets both the slow driver and
the demographic operators call it. Leaf necromass is pooled with **storage** necromass (both
canopy-associated and labile-eligible; dead seed and seedling arrive here) and split by `f_labile_leaf`;
...
X^{n+1}_i = X^n_i + u_i + \sum_j a_{ij}\,d_j\,X^n_j \qquad(5)
```
...
R_h^{\mathrm{day}} = \sum_i u_i - \Big(\sum_i X^{n+1}_i - \sum_i X^{n}_i\Big) \qquad(6)
```
...
the number that says whether freezing the pool across the slow step is sound. It is scale-free (the
flux is linear in the pool, so $\lambda$ does not grow as a pool empties, which is why bare-ground
spin-up works); a daily step on a mature stand withdraws a few tenths of a percent of the fastest pool,
approaching 1 means the step is too long for that pool, and past 1 forward Euler drives it negative.

## 7. Why the accumulated integral, and not a daily-mean scalar

$\xi$ is a **product of nonlinear functions** of temperature and moisture, both of which swing over a
day, so by Jensen's inequality
$`\langle f_T(T) f_\theta(\theta)\rangle \neq f_T(\langle T\rangle)\,f_\theta(\langle\theta\rangle)`$.
A daily step that recomputed $\xi$ from daily-mean state would debit the pools by a *different* number
than the sub-daily flux actually delivered to the atmosphere, and ecosystem carbon would drift while
every individual budget still closed. Integrating $\xi$ as it happens removes the gap by construction
rather than bounding it; the tests run a diurnal cycle through both paths and assert that the
accumulated one closes to round-off while the daily-mean one does not.

## 8. The fast/slow seam
...
integrates $`\xi_j`$ into the per-patch `xi_accum` (the $`\xi^{\mathrm{int}}`$ of eq (5)) and $`R_h`$
...
multi-millennial integration: setting $`dX/dt=0`$ in eq (1) with mean input $`\bar u`$ and mean scalar
$`\bar\xi`$,

```math
0 = \bar u + A\,\bar\xi K X_{ss} \quad\Longrightarrow\quad
M\,X_{ss}[\mathcal{A}] = -\bar u[\mathcal{A}], \qquad
M = \big(A\,\bar\xi K\big)[\mathcal{A},\mathcal{A}], \qquad \mathcal{A}=\{j: K_j>0\} \qquad(7)
```
...
scheme 5's non-triangular back-transfers as well as the default block, whose diagonal dominance keeps
it well conditioned. A run can start here (`[soil_carbon].spinup_steady`), seeding every patch from
eq (7) with a scalar climatological $`\bar\xi`$ and a constant litter estimate — with two caveats:
those means are **prescribed**, not fed back from a live canopy, and a mean-driver equilibrium discards
the covariance of periodic forcing, so it only approximates the true attractor.

## 10. Traceability diagnostics

`soil_carbon_diagnostics` is pure post-processing off the already-assembled operator — no new state —
reusing the §9 active-block solve to say where the system is *heading*:

```math
X_c = -\big(A\,\xi K\big)^{-1}u, \qquad X_p = X_c - X, \qquad
\tau_E = \frac{\sum_i X_{c,i}}{\sum_i u_i}\Big/\texttt{yr\_day} \qquad(8)
```

$`X_c`$ is the **storage capacity**, the equilibrium the patch is chasing under today's climate and
litter input — a moving attractor, not a fixed point; $`X_p`$ is the **storage potential**, the
remaining sink (negative means a source); $`\tau_E`$ is the ecosystem **residence time** in years,
stock over throughput, on the active block alone. The transient relaxes toward $`X_c`$ at a rate set by
$`A\,\xi K`$, which makes the trio a decomposition of *why* two runs store different carbon.

## 11. Audits, and the difference between conservation and plausibility

Every step fills a `soilc_audit_t`: `litter_in_matrix`, `dC_pool`, `rh_out`, `rh_seam_gap` (§8),
...
$`e_r\,\xi^{\mathrm{int}}K X^n`$ and compare against $`\sum u-\Delta X`$ — that is what catches dropped
litter or a misattributed transfer; comparing eq (6) against itself cannot.

Conservation is not plausibility — a runaway that conserves carbon closes every budget while moving
absurd amounts of it. `soil_carbon_bad_pool` answers the question a budget cannot: 0 when every pool is
physically possible, otherwise the **index** of the first that is not (NaN, negative beyond a round-off
...
Euler step produces them routinely — and the ceiling catches divergence rather than policing science.
...
- **DAMM.** `heterotrophic_respiration_damm` implements Davidson's dual Arrhenius/Michaelis-Menten
  ...
  key or a production caller — the matrix form (§8) is what the fast loop respires, and the tests are
  their only consumer.
...
- **No fire.** The above/below split exists so a fire class can consume the above-ground pools, but no
  such class exists; treefall is the only disturbance.
...
| Key | Meaning |
|---|---|
| `soil_carbon_on` | master switch for the subsystem (default on) |
| `decomp_scheme` / `step_solver` | `"ed2"` or `"century5"` / `"euler"` or `"expm"` |
| `k_fast`, `k_struct`, `k_micr`, `k_slow`, `k_pass` | baseline decay rates $`[\mathrm{yr^{-1}}]`$ |
| `er_fast`, `er_struct_nonlig`, `er_struct_lig`, `er_slow`, `er_pass`, `e_lignin` | respired fractions + the lignin brake, eqs (3)–(4) |
| `er_micr_*`, `xsand`; `fx_micr_pass_*`, `fx_slow_pass_*`, `xclay` | scheme-5 texture control (sand → microbial respired fraction, clay → transfers to passive) |
| `rh_q10`, `rh_t_ref`, `resp_temp_increase`, `resp_temp_ref` | temperature response, eq (3) |
| `resp_opt_water`, `resp_water_below_opt`, `resp_water_above_opt` | moisture/oxygen response, eq (3) |
| `n_cycle_on`, `c2n_*`, `n_immobil_supply_scale` | nitrogen (shaped in, inert — §12) |
| `spinup_steady`, `spinup_xi`, `spinup_{labile,struct}_*` | SASU cold start, §9 |
...
| pool plausibility predicate | `meds_soil_biogeochem`: `soil_carbon_bad_pool`, `soil_carbon_pool_name` |
...
| output variables; tests | `soilc_*_site`, `soilc_total_patch`, `rh_site` (`GRP_CARBON`); `test/test_soil_biogeochem.f90`, `test/test_biogeochem_dynamics.f90`, `test/test_column_co2.f90` |
